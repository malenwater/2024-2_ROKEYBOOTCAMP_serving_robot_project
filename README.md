...
|[최범석](https://github.com/ausudu)|코드 구조 파악: 코드 구조가 복잡해서 처음에는 이해하는 데 시간이 걸렸다. 명확한 설계가 부족해 수정과 유지보수가 어려웠다. ChatGPT에 의존: 문제 해결을 위해 자주 ChatGPT를 사용했지만, 때때로 그것만으로는 완전한 해결책을 얻기 어려웠다. 의존성이 커져서 스스로 생각하고 해결하는 능력이 부족해졌다. 팀원과의 의사소통 부족: 팀원들과의 커뮤니케이션이 부족해 작업효율이 떨어졌다|코드 구조 파악: 문제를 해결을 위해 우선 기본적인 개념을 먼저 학습하자. 코드 작성 전 전체적인 설계를 미리 구상하고 작성하도록 하자 ChatGPT에 의존:  기본적인 틀을 스스로 작성하고 ChatGPT는 보조적인 도구로 사용하도록 하자 팀원과의 의사소통 부족: 정기적인 회의를 통해 진행 상황을 공유하고, 작업 분배를 명확히, 팀원들과 자주 피드백을 주고받는것이 중요한거같다.|일일 메뉴, 일일 매출, 메뉴별 매출 기능 서비스, 키오스크 UI와 주방 UI의 Node 간 통신 구축|
